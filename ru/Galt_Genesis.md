...
1. При деплое контракта происходит mint N GALT ( ERC-20 ) токенов на multisig генезис фонда.
...
5. На момент окончания времени T, если на балансе GALTGenesis multisig остались GALT токены, последний пользователь, который купил GALT токены инициирует метод burn, который уничтожает все оставшиеся токены.
