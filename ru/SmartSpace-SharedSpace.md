...
При создании контракта:
    * Внутри контракта выпускается токен объемом 5000 GALT.
...
3. Пользователь A перечисляет свой Space токен на контракт.
	...
	* Пользователь А получает всю репутацию токена 2500 GALT, более детальное распределение:
	    * 1500 GALT - реальная собственность пользователя А (внутренние токены хранятся на адресе пользователя А)
	    * 1000 GALT - нераспределенная репутация (внутренние токены хранятся на адресе контракта, но репутация распределена пользователю А)
...
Владалец контракта сразу же может вывести GALT токены, полученные при продаже внутренних токенов.
...
* Пользователь делает заявку, сколько внутренних токенов он хочет продать и по какой цене в GALT.
* Первые 7 дней (неменяемый параметр) выкупить внутренние токены могут только текущие владельцы внутренних токенов.
* После 7 дней выкупить внутренние токены может любой пользователь.
* Продавец может создать whitelist адресов, которым разрешено купить внутренние токены
...
* Объем внутренних токенов в случае штрафов не меняется.

### Голосование
* При голосовании учитывается объем внутренних токенов конкретного пользователя.
