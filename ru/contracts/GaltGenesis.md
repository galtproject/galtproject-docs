...
6. Если Генезис закончен и цель с учетом погрешности достигнута, то контракт Генезиса GALT Территории создает токены GALT, вызывая метод mint() контракта GaltToken и становится их владельцем. Контракт GaltGenesis по id заявки и по соответствующей этой заявке записи в контракте SPACEAuctionRegistry запускает соответствующий контракт Аукциона земли. Контракт GaltGenesisRegistry выпускает SPACE токен Территории, владелец - адрес контракта PACEAuctionRegistry.  Газ платит последняя транзакция.
