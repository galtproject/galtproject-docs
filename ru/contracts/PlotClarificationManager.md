...
* REJECTED - один из валидаторов не одобрил заявку
...
* REJECTED - валидатор не одобрил заявку
...
* Метод возможен только для заявок
    * в статусах PACKED, REJECTED, REVERTED, REVOKED
...
* Метод доступен только для заявок
    * в статусах NEW, PAYMENT_REQUIRED, REVERTED, REJECTED, PACKED
...
* Статус заявки меняется на REJECTED - если хотя бы один из валидаторов решил отказать и сменил статус роли на REJECTED
* Статус роли переоходит в REJECTED
...
* Метод доступен только для заявок
    * в статусах PACKED, REJECTED, REVERTED
    * флага TOKEN_WITHDRAWN == true
    * флага GAS_DEPOSIT_WITHDRAWN == true
...
8. Валидатор злонамеренно не подтвердил корректную заявку и
    1. Перевел ее в статус REVERTED - по жалобе от заявителя валидатор может быть снят с заявки (#resetApplicationRole()) и удален (Validators.removeValidator()) для дальнейшего рассмотрения.
    2. Перевел ее в статус REJECTED - по жалобе от заявителя валидатор может быть удален (Validators.removeValidator()) для дальнейшего рассмотрения. Комиссия заявителю не возвращается, необходима подача заявки заново.
9. Валидаторы не хотят работать над заявкой TBD
