...
### Новые Термины
- `Заявка на Территорию` - запись в контракте CreateTerritory, соответствующая предложению о включении `Территории` в `Проект`.
...
При подаче `Заявки на Территорию` нужно следовать следующим условиям:
- На момент подачи `Заявки на Территорию` `Территория` принадлежит `Владельцу Территории`.
- В `Заявке на Территорию` могут быть указаны несколько токенов SPACE.
- Для осуществления продажи `Территории` - необходимо передать её во владение `SPACEAuctionRegistry`: 
  - если токен Территории будет участвовать в Аукционе без разделения, то он полностью будет переден позже от контракта  `SPACEAuctionRegistry` на конкретный контракт `SPACEAuction`;
  - если токен Территории будет разделен для Аукциона, то <тут надо думать>.

Пользователь может купить `Территорию` или отдельные `Земельные участки` на Аукционе двумя способами:

1. В Аукционе участвуют Токены SPACE так, как они были указаны в `Заявке на Территорию`, без разделения.

Пример: В `Заявки на Территорию` были указаны 2 токена SPACE, которые были переданы контракту SPACEAuctionRegistry.
...
`Владелец Территории` решил, что эти токены будут продаваться, как единое целое. После запуска конкретного контракта `SPACEAuction_465` они были переданы контракту `SPACEAuction_465`.
...
В `Заявки на Территорию` был указан 1 токена SPACE, который был передан контракту SPACEAuctionRegistry.
...
### Сценарий №1 "Создание заявки на новую Территорию на выкуп через дополнительную эмиссию GALT. В заявке указываются готовые токены SPACE Упаковок. Аукцион будте происходить только по этим токенам".
...
1. `Оператор ввода` может вызвать метод CreateNewTerritoryProposal() и создать заявку на продажу новой  `Территории`. В `Заявке` `Оператор ввода` указывает:
...
| Минимальная ставка для участка в GALT| 1000 |  |
| Стоимость Территории в Эфирах| 15 000 | Эта сумма, которую хочет получить владелец за все Токены SPACE, указанные в Заявке.|
| Количество выпускаемых под Территорию GALT| 15 000 000 | Эта сумма GALT, которая будет выпущена.|
...
| Размер вознаграждения Оператора ввода Территории | 1350 | |
...
| Адрес задеплоенного контракта TerritoryCrawdsale | адрес |Это контракт, который в случае принятия Заявки будет добавлен в контракт GaltGenesisRegistry|
