...
4. Если заявка была принята так же CreateTerritory добавляет адрес контракта аукциона земли из заявки в контракт SPACEAuctionRegistry. То есть каждый раз заранее задеплоенный контракт добьавляется в SPACEAuctionRegistry на конкретную территорию.
