...
Если в проекте еще нет Владельцев земли и токенов GALT то решение о вводе новой территории должны принять создатели проекта.
Если время жизни заявки истекло и не было принято положительное решение, то заявка считается отклоненной.

Владелец земли должен подписать заявку и указать флаг, который определяет, что межевание будет происходить заранее и все SPACE - токены будут созданы им самостоятельно для аукциона. Если он не подписал заявку, то заявка считается не принятой.

3. Если заявка была принята, то контракт CreateTerritory в контракте GaltGenesisRegistry создает запись с адресом контракта Генезиса GALT Территории из заявки на сбор средств в Эфирах для покупки Территории и пополнения Фонда. Контракт продлится 90 дней. Каждый может отправить Эфиры на контракт и получить в обмен GALT по фиксированному курсу после достижения цели. Если за 90 дней не будет собрано 15 000 Эфиров, то Эфиры будут возвращены владельцам. Подробнее - см. Документацию контракта.
4. Если заявка была принята так же контракт CreateTerritory добавляет адрес контракта Аукциона земли из заявки в контракт SPACEAuctionRegistry. То есть каждый раз заранее задеплоенный контракт добавляется в SPACEAuctionRegistry на конкретную территорию.

5. Если Владелец земли выставил флаг о том, что будет задано начальное межевание, то Владелец земли может вызвать метод и создать все SPACE - токены заранее.

6. Далее - Контракт GaltGenesis.
