...
- На момент подачи `Заявки на Территорию` токены SPACE, которые соответствуют `Территории` принадлежат `Владельцу Территории`.
...
- Для осуществления продажи `Территории` - необходимо передать её во владение `SPACEAuctionRegistry`: 
  - если токен(или токены) Территории являются `Упаковками` и будут участвовать в Аукционе без разделения, то он(они) полностью будет передан позже от контракта  `SPACEAuctionRegistry` на конкретный контракт `SPACEAuction`;
...
1. В Аукционе участвуют Токены SPACE так, как они были указаны в `Заявке на Территорию`, без разделения. Токены являются `Упаковками`.
...
В `Заявки на Территорию` был указан 1 токен SPACE, который был передан контракту SPACEAuctionRegistry.
