...
## Роли заявки
Роли валидаторов, используемые в заявках, жестко прописаны в контракте PlotValuation. Список ролей в контракте Validators, который задается вручную, должен четко ему соответствовать
...
* MATCH - оффер выбран, ожидается перевод токенов на контракт, закрыто создание новых офферов и изменение текущих
* ESCROW - предложения совпадают и все токены переданы на контракт, ожидается вывод токенов, закрыто создание новых офферов и изменение текущих
* CLOSED - продажа закрыта

## Статусы офферов
* OPEN - оффер открыт
* MATCH - продавец выбрал этот оффер для дальнейшей работы с ним
* ESCROW - токены поступили, ожидается их вывод
* CLOSED - продажа закрыта
...
* Разрешен только для заявок в статусе OPEN
* Разрешен только для офферов в статусе OPEN
* Если предложения продавца и покупателя равны, тот статус оффера и статус заявки устанавливается в MATCH и цену больше изменить нельзя

#### #attachSpaceToken()
Продавец прикрепляет свой токен к паре заявка/предложение
* Разрешен только для заявок в статусе MATCH
* Разрешен только для предложений в статусе MATCH
* Может быть прикреплен токен, id которого изначально указано в заявке
* Если на контракте уже присутствует платеж и назначен кастодиан , то:
    * Статус заявки меняется на ESCROW
    * Статус оффера меняется на ESCROW

#### #withdrawSpaceToken()
Продавец отзывает свой Space токен с контракта
* Разрешен только для заявок в статусе MATCH
* Разрешен только для предложений в статусе MATCH (Токены оплаты еще не переведены)
* Статус заявки и статусы оффера не меняются

#### #claimReward()
Продавец выводит вознаграждение за SpaceToken
* Разрешен только для заявок в статусе ESCROW
* Разрешен только для предложений в статусе ESCROW (Space токен еще не переведен)
* Если покупатель уже вывел Space токен - статус заявки и оффера меняются на CLOSED

#### #cancelSale()
Продавец закрывает продажу токена
* Разрешен только для заявок в статусе OPEN и MATCH
* Разрешен только в случае, если И токен И оплата еще не переведены на контракт
...
* Если на контракте уже присутствует Space токен и назначен кастодиан, то:
    * Статус заявки меняется на ESCROW
    * Статус оффера меняется на ESCROW
...
* Разрешен только для заявок в статусе MATCH
* Разрешен только для предложений в статусе MATCH (Space токен еще не переведен)
* Статус заявки и статусы оффера не меняются
...
* Разрешен только для заявок в статусе ESCROW
* Разрешен только для офферов в статусе ESCROW (Space токен еще не переведен)
...
* Запрос является конкурентным с запросами по выводу денег/токена и может быть опережен ими
* Статус оффера меняется на APPROVED

#### #checkCustodian()
* Разрешен только для заявок в статусе MATCH
* Разрешен только для заявок, у которых и Space токен и токены оплаты уже переведены на контракт
* Если кастодиан в контракте PlotCustodianManager зарегистрирован:
    * Статус заявки меняется на ESCROW
    * Статус оффера меняется на ESCROW
...
* Заявка переходит в статус CANCELLED
