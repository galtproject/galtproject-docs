...
* Указывается список адресов-кастодианов для привязки/отвязки
* Указывается id токена упаковки
* Указывается действие - привязать или отвязать кастодианов
...
#### #accept()
Кастодианы из списка на изменения соглашаются над этим действием

* Статус заявки должен быть SUBMITTED
* После того, как все (без исключения) изменяемые кастодианы согласятся это изменение, cтатус заявки меняется на ACCEPTED

#### #ready()
Уже назначенные кастодианы блокируют работу над заявкой

* Статус заявки должен быть ACCEPTED
* После того, как все (без исключения) изменяемые кастодианы согласятся это изменение, cтатус заявки меняется на LOCKED

#### #revert()
(Один из кастодианов из списка на изменение) или (один из уже назначенных кастодианов)  НЕ соглашается над это действие
* Статус заявки должен быть SUBMITTED или ACCEPTED
* Статус заявки меняется на REVERTED
* Сбрасываются все голоса как кандидатов на изменения так и существующих кастодианов

#### #attachDocuments()
Любой кастодиан прилагает массив хешей IPFS документов
