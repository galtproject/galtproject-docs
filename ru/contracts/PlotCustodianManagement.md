...
* Действие:
  * ATTACH_CUSTODIAN - привязать кастодиана
  * DETACH_CUSTODIAN - отвязать кастодиана
...
* `COMPLETE` - выполнена (токен переведен обратно заявителю)
...
* Метод возможен только для заявок в статусе COMPLETE
...
* Токен упаковки должен иметь оценку в контракте PlotValuation
...
* Флаг WITHDRAWAL_APPROVED должен быть равен true
* Флаг TOKEN_WITHDRAWN должен быть равен false
* Проставляется флаг TOKEN_WITHDRAWN

## Интерфейс для валидаторов
#### #lock()
Кастодиан соглашается работать над заявкой
* Статус заявки должен быть SUBMITTED
* Статус заявки меняется на LOCKED
...
* Делается запись в реестре о привязке токена упаковки к адресу кастодиана
* Проставляется флаг WITHDRAWAL_APPROVED - разрешен вывод токена заявителем
* Статус заявки меняется на COMPLETED

#### #claimValidatorReward()
Валидатор забирает свое вознаграждение

* Статус заявки должен быть COMPLETED
