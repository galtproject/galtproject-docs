...
В этом случае, если Владельцем земли были созданы SPACE токены для конкретного Аукциона земли, то любой может вызвать метод и контракт SpaceAuction, как владелец токенов вызовет метод burn().
