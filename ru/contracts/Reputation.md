...
Таким образом мы предоставляем владельцу токена методы на перераспределение его (токена) репутации. Дергать методы перераспределения могут:
...
`Необходимо продумать`. Это может быть как увеличение (в случа, когда пользователь добровольно пополняет репутацию с целью увеличения ее веса при голосовании), так и уменьшение (штрафы).
