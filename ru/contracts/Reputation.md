...
## Цели контракта
Для решения этой проблемы, мы кешируем кол-во GALT, заложенных за 'Земельные участки' Space токены) и называем этот показатель `Репутация`.
...
3. Пользователь А c помощью метода `getPreferredFund` узнает, что пользователь B предопочитает, чтобы назначаемая ему
