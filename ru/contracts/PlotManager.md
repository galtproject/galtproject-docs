...
Заявка записывается в Контракт. Пользователь оплачивает невозвратную комиссию за рассмотрение заявки. Сейчас это комиссия 1 ETH.
...
Проверяет, что границы Участка совпадают и Геохеши не выходят за границы земельного участка. Приложение выдает сообщение, что Хеш Фамилии, Имени и ID совпадают и что границы участка совпадают. Появляется кнопка `Утвердить заявку`
...
### Сценариц №2 - Добавление Геохешей в заявку
...
### Сценариц №3 - Разбор упаковки Заявителем
...
4. После того, как счетчик геохешей становится равным 0, статус заявки меняется не `Неактивная`.

|Параметр|Значение|
|--------|--------|
|id Заявки|DBn4y2a......35b1|
|Статус заявки|Неактивная|

### Сценариц №4 - Разбор упаковки Валидатором

1. Валидатор видит в интерфейсе все заявки, которые имеют статус "Отклонена" и которые были назначены ему.
