...
Заявитель добавляет геохеши в заявку

* Разрешен только для заявок в статусе `NEW` или `REVERTED`
* Разрешен только для заявителя
* Передаются данные для заявки
* Передается массив геохешей. Размер массива ограничен лишь размером блока.
* Передаются массивы соседних геохешей и их направлений. На данный момент эта информация не используется.
* Суммируется газ, потраченный на транзакцию. В дальнейшем, заявителю необходимо будет внести депозит
 в размере суммы учтенного газа по цене, установленной оператором `fee manager`.
* Суммируется площадь участка в зависимости от размера геохеша
* Статус заявки не меняется

#### #removeGeohashesFromApplication()
Заявитель или любой валидатор удаляют геохеши в заявку

* Разрешен только для заявок в статусе `NEW` или `REVERTED`
* Разрешен для вызова заявителем, оператором заявки и для любоым из валидаторов
* Передаются данные для заявки
* Передается массив геохешей. Размер массива ограничен лишь размером блока.
* Передаются массивы соседних геохешей и их направлений. На данный момент эта информация не используется.
* Вычитается площадь участка в зависимости от размера геохеша
* Статус заявки не меняется
...
* Оплачивается комиссия за рассмотрение заявки в ETH или GALT. Размер комиссии долеж быть равен или
    должен превышать минимально рассчетную комиссию. (На данный момент, функционал не протестирован).
* Оплачивается депозитв ETH, возващаемый после разборки заявки заявителем. Размер депозита должен четко
    соответствовать рассчетному.
...
* Депозит повторно не оплачивается
...
* Разрешен только для заявок в статусе `DISASSEMBLED_BY_APPLICANT`
* Разешен для вызова только заявителем
* Заявителю возващается вся уплаченная им комиссия
* Статус заявки меняется на `REVOKED`

#### #claimGasDepositByApplicant()
Заявитель выводит депозит за газ

* Разрешен только для заявок в статусе `APPROVED` или `DISASSEMBLED_BY_APPLICANT`
* Разешен для вызова только заявителем
* Заявителю возващается оплаченный депозит в ETH
* Статус заявки не меняется
...
#### #approveApplication()
Валидатор одобяет заявку по своей роли
...
#### #revertApplication()
Валидатор возвращает заявку по своей роли
...
* Разрешен только для заявок в статусе `APPROVED` или `DISASSEMBLED_BY_VALIDATOR`
* Разрешен только для ролей в любом статусе
* Разрешен для вызова только тем валидатором, который заблокировал для себя работу на ролью
* Статусы роли и заявки не меняются

#### #claimGasDepositByValidator()
Валидатор выводит депозит за газ

* Разрешен только для заявок в статусе `DISASSEMBLED_BY_VALIDATOR`
* Разешен для вызова любым валидатором
* Валидатору возващается весь оплаченный заявителем депозит в ETH
* Статусы роли и заявки не меняются
