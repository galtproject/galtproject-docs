...
|Комиссия Кастодиана в  GALT|Float|0|Комиссия раз в месяц, которая должна быть перечислена Кастодиану от Владельца токена, устанавливается Заявителем|
...
1. Ежемесячно происходит начисление комиссии Кастодиана в контракте. 
...
5. Для начисления комиссии Кастодиан нажимает кнопку "Начислить комиссию" и подписывает транзакцию с вызовом метода PlotManager.

## Сценарий UI : Начисление долга на комиссию Кастодиана
1. Кастодиан в меню нажимает кнопку "Начислить долг".
2. Происходит проверка в интерфейсе, что есть просрочка, то есть "Начисленная комиссия не равно Оплаченной комиссии".
3. Происходит расчет в интерфейсе размера начисления долга по данным из контракта.
4. Создается транзакция с вызовом метода PlotManager. В методе есть проверка , что "Начисленная комиссия не равна Оплаченной комиссии". Если проверка пройдена, то происходит начисление долга по данным из контракта и увеличивается переменная "Начисленный долг".
...
1. Кастодиан может вывести комиссию в разделе Меню Кастодиана - Мои Участки.
2. Отображается список участков, для которых пользователь является Кастодианом.
3. У каждого участка отображается: начисленная комиссия, перечисленная комиссия от владельца токена, выведенная комиссия, долг, проценты по долгу.
3. Кастодиан нажимает кнопку, создается транзакция, которая вызвает метод PlotManager и выводит перечисленную комиссию с контракта на адрес Кастодиана. Переменная "Выведенная комиссия" увеличивается.
