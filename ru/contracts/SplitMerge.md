...
В контракте SplitMerge добавляется запись в маппинг CustomPlotBorders, который содержит соответствие `id уникального участка` - `координата вершины`. Эта координаты вершин носят справочный характер.
