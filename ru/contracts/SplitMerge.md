...
7. SplitMerge разделяет `Репутацию` токена SPACE [sezu01](http://explorer.galtproject.io/map/#sezu01) на 32 части используя метод transferFromToken() контракта Reputation и начисляет на 32 созданных токена SPACE. Одновременно токен sezu01 передается контракту SplitMerge, а 32 токена SPACE передаются Пользователю А. 
...
6. SplitMerge суммирует Репутацию 32 токенов sezu0b0, sezu0b1, sezu0b2 итд. и переносит ее при помощи метода transferFromToken() контракта Reputation на токен sezu0b. И одновременно передает токен sezu0b Пользователю Б. А токены sezu0b0, sezu0b1, sezu0b2 итд. получает контракт SplitMerge.
