...
* CLOSED - Продажа закрыта успешно. Дальнейшая работа с офферами невозможна.
* CANCELLED - Продажа отменена. Дальнейшая работа с офферами невозможна.
...
* Заявка не может быть создана, если уже существует другая заявка с этим же Space токеном и ее статус не равен `CANCELLED`
...
* Если покупател уже перевел платеж, меняет статус на ESCROW

#### #withdrawSpaceToken()
Продавец отзывает свой Space токен с контракта
* Разрешен только для заявок в статусе CLOSED
* Разрешен только для предложений в статусах LOCKED
* Статус ордера не меняется
* Если платеж уже выведен, статус оффера меняется на EMPTY
...
* Разрешен только для ордеров в статусе LOCKED
* Разрашен только для офферов в статусе EMPTY
* Статус заявки устанавливается в OPEN
* Статус оффера не меняется
...
* Разрешен только для заявок в статусе OPEN
* Покупатель указывает свой бид в валюте заявки
...
#### #changeOfferBid()
Покупатель изменяет свой бид по покупке участка
* Разрешен только для заявок в статусе OPEN
...
* Если продавец уже перевел Space токен, меняет статус на ESCROW
...
* Статус ордера не меняется
* Если Space токен уже выведен, статус оффера меняется на EMPTY
...
* Статус роли аудитора меняется на PENDING
...
#### #emptySaleOffer()
Любой адрес может перевести оффер из статуса CANCELLED в статус EMPTY, если на нем отсутствуют привязанные токены оплаты и Space токен

* Разрешен только для заявок в статусе LOCKED
* Разрешен только для офферов в статусе CANCELLED
* Статус заявки не меняется
* Статус оффера меняется на EMPTY
...
* Разрешен только если статус аудитора PENDING или REJECTED
...
* Статус роли аудитора меняется на REJECTED
...
* Статус роли аудитора меняется на APPROVED
* Статус ордера не меняется
* Оффер переходит в статус CANCELLED

#### #claimValidatorReward()
Аудитор выводит свое вознаграждение
* Разрешен только для ордеров в статусе CANCELLED и CLOSED
* Разрешен только для офферов, в которых учавствовал этот аудитор
* Если было несколько итераций процесса аудита - вознаграждение может забрать только последний аудитор

## Интерфейс для GaltSpace
Fee manager выводит вознаграждение GaltSpace
* Разрешен только для ордеров в статусе CANCELLED и CLOSED
* Если в оффере потребовалось участие аудитора - GaltSpace делит с ним вознаграждение
* Если аудитор не участвовал в оффере - 100% вознаграждение получает GaltSapce.
