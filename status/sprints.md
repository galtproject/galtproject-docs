...
## Web3 Application
  - Перевести Frontend на новую версию контрактов PlotManager и Validators
    - Получение списка ролей валидатора
    - Получение списка ролей зявки
    - Получение статуса по каждой из ролей в заявке:(одобрена или не одобрена)
    - Указание роли валидатором при одобрении заявки
    - Подача заявки GALT или ETH (селектор и предупреждение с размером комиссии)
    - Возможность установки большей комиссии
    - Редактирование деталей завявки Апликантом при создании заявки и возвращении от валидатора
    - Админка с указанием ролей и долей в эфирах и галтах
    - Получение списка ролей по типу заявки(тип указан в PlotManager)
    - Сброс залоченного валидатора владельцем контракта
    - Проверить на изменение полей заявки
    
## Контракты
  - SpaceDex
    - SetPlotCustodianManagement
      - Установка адреса контракта для получения цены за Space токен
    - ExchangeSpaceToGalt
