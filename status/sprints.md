...
* Добавление валидаторов - добавлять локацию в виде геохеша, добавлять тип валидатора - 120 мин
* Установка необходимого количества валидаторов
    ...
    * Пересчет кол-ва необходимых подтверждений c учетом типов валидаторов в соответствующих методах 180 минут
...
## TypeScript Telegram Bot (20 ч 30 мин)
 ...
 - Маркетинговый бот (общего назначения)
   ...
   - Частичное голосование своей репутацией за кандидата в делегаты [#410](https://github.com/galtspace/WIJG/issues/410), 360
     - В метод vote добавить параметр кол-ва репутации и обработку его
     - Перестроить маппинг voterForCandidate для хранения там множества кандидатов для каждого voter'а
     - Добавить маппинг хранения того, каким кол-вом репутации и за кого проголосовал пользователь
     - В маппинг voter2Reputation записывать то кол-во репутации, которое обозначил пользователь
     - Не позволять пользователю проголосовать итоговой суммой больше, чем сумма его текущей репутации
     - Установить максимальное кол-во кандидатов, за которое может проголосовать пользователь = 7
     - При действии изменения репутации в контроллере вызывать отмену всех голосов пользователя за кандидатов с изменением итоговых голосов за кандидатов
     - Добавить в интерфейс вопрос об кол-ве репутации, которым пользователь хочет проголосовать за кандидата
