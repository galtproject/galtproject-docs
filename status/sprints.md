...
### Доработки
  ...
  - Внедрение WebWorker для распаралеливания различных процессов в браузере пользователя
    - Аппроксимация
    - Совершение множества транзакций сгенерированным в браузере кошельком
    - Сортировка геохешей по наличию соседей(для отправки транзанкций по добавлению)
    - Слежение за отправленной пользователем транзакцией для уведомления 
  - Получение результата выполнения транзакции после её отправки(например номер заявки после её создания)
  - Перенести уведомления пользователю вправо
  - Протестировать и исправить возможные проблемы в testnet56
  - Пренести верхние вкладки в новое меню слева
    - Сделать меню сворачивающимся
    - Сделать разделы в меню: Валидирование, Администрирование и т.д.
  - При проверке геохешей упаковки - если есть недостающие геохеши: показать их на карте в окне предупреждения выделив цветом среди других геохешей упаковки
  - Поставить ограничение на максимальный размер геохеша для регистрирования территории чтобы избежать злоупотребления
  - В интерфейсе проверять площадь указанного пользователем контура и предупреждать в соответствии с площадью - какую точность он может указать, а какая приведет к долгим расчетам.
  - Проверять в какой сети MetaMask чтобы предупреждать пользователя о том что ему нужно переключиться на другую сеть
  - Проверить кнопку создания транзакции: почему она может быть не активна
  - Отловить баг с созданием заранее определенной территории и сломанной транзакцией approve
  
### Новый функционал
  - Верхнее меню пользователя: Баланс GALT, ETH, SPACE
  - Добавить вариант отправки транзанкций с помощью персонального сгенерированного кошелька пользователя в браузере, на который пользователь может отправить ETH, чтобы кошелек с помощью WebWorker в обход метамаск от имени пользователя подписал множество необходимых транзакций для регистрации участка.
    - При необходимости отправки множества транзанкций спрашивать пользователя: отправить ли с помощью `Встроенного кошелька`
    - Генерировать адрес и приватный ключ в браузере и показывать адрес пользователю с предложением отправить на него ETH или нажать кнопку для отправки через MetaMask
    - Рассчитывать необходимое количество ETH для отправки нужных транзанкций
    - Следить за изменением баланса кошелька чтобы обновить при поступлении средств и вывести пользователю уведомление
    - Отправлять множество необходимых транзанкций с помощью приватника
    - В верхнем меню пользователя добавить пункт `Встроенный кошелек` и его баланс в ETH
    - В меню `Встроенный кошелек` показывать количество транзакций, например 10/100 и обновлять его при совершении новой транзакции
    - После завершения отправки всех транзакций - отправлять ETH обратно пользователю
  - Добавить иконку со справкой по разрядам геохеша - какой разряд какие имеет размеры
  - Установить в контракте минимальное значение которое необходимо чтобы заявка прошла, но если хочешь чтобы быстрее твою заявку рассмотрели - плати больше, и валидаторы будут как таксисты сидеть и брать в работу в первую очередь те заявки за которые больше заплатили. И в интерфейсе при этом пользователю можно показывать минимальную цену и оптимальную на основе последних 100 заявок
    - Показ оптимальной цены комиссии на основе последних 100 заявок
    - Предложение пользователю внести большую сумму чем минимальную
    - Показ валидаторам последних 100 заявок, отсортированных по комиссии
    - Возможность изменить число показываемых заявок
  - Раздел Администрирование
    - Добавление и удаление валидаторов
    - Изменение минимальной комиссии в контракте PlotManager
    - Получение вознаграждения с комиссий для GaltSpace для каждого из участков
      - Список участков с кнопками: получить комиссию
    - Устанновить комиссию за обмен GaltDEX
    - Отображение текущего доступного обьема для вывода комиссии из GaltDEX и вывод
...
* Установка необходимого кол-ва валидаторов
    * Метод установки setRequiredValidators(по умолчанию 1) 30 мин
    * Метод approveApplication() 120 минут
    * Метод revokeApproval() 120 минут
