...
### Доработки (3.5 часа)
  ...
  - **Перенесено на следующий спринт** Очередь в WebWorker для большого количества транзанкций с целью отправки последовательно и ожидания подтверждения транзанкций для отправки новых
  ...
  - Оформлять отправление нескольких транзанкций с внутреннего кошелька в одну транзанкцию
  - **Перенесено в следующий спринт** Логи для пользователя на экране блокировки: какой статус у транзанкций в текущий момент
...
## Web3 Application (31.75 часов)
  ...
  - Очередь в WebWorker для большого количества транзанкций с целью отправки последовательно и ожидания подтверждения транзанкций для отправки новых
    ...
    - Обеспечить логику и ограничение на одновременную отправку нескольких транзанкций, 60 мин
    - Повторная отправка транзакции при первом реверте, с повышенным количеством газа, 30 мин
  ...
  - Логи для пользователя на экране блокировки: какой статус у транзанкций в текущий момент
    - Использование localStorage для хранения хешей транзанкции и данных об операции, 120 мин
    - Заменить проверку количества геохешей через balanceOf на проверку статуса транзанкции, 120 мин
    - Хранить и отображать общее количество транзакций в очереди и количество выполненных транзакций, 60 мин
    - Отправка статусов по транзанкциям из WebWorker в интерфейс, 30 мин
    - Выводить список транзанкций с хешами в интерфейсе, 60 мин
    - Показывать статус по каждой транзакции в интерфейсе, 60 мин
  - Блокировать экран в случае если rpc-server не соответствует базовому
    - Верстка и подключение как плагина нового экрана блокировки, 60 мин
    - Проверять соответствие rpc-server на соответствие допустмым и блокировать экран если не соответствует, 30 мин
    - Указывать информацию с видео туториалом по установке корректного rpc-server, 15 мин
    - Хранить и выводить список допустимых rpc-servers, 30 мин
    - Разблокировать экран после указания правильного rpc-server, 30 мин
  - Раздел Администрирование
    - Добавление и удаление валидаторов с их ролями, 60 мин
    - Изменение минимальной комиссии и коэффициента в контракте PlotManager, 30 мин
    - Получение вознаграждения с комиссий для GaltSpace для каждого из участков, 30 мин
      - Список участков с кнопками: получить комиссию
  - Указание и отображение информации об отклонении и возвращении заявки валидатором: Валидатор должен написать текстом - что в заявке не правильно, 30 мин
  - Отправка ETH на новый адрес внутреннего кошелька при перегенерации кошелька, 30 мин
  ...
  - Убрать чекбокс Активен, при каждой необходимости использования кошелька - задавать вопрос пользователю, 60 мин
  - Каждый раз при согласии пользователя выполнить операции внутренним кошельком - генерить новый кошелек, 30 мин
  - После каждой операции по сборке, разборке участков и заявок в явной форме требовать от пользователя отзыва allowance от внутреннего кошелька на операции с Space токенами и блокировать экран, 120 мин
  - При перезагрузке страницы проверять, что allowence отозван, 30 мин
...
## Backlog
  ...
  - Вывод ошибок в логи в случае провала транзанкции
      ...
      - Вывод ошибки пользователю на экран блокировки, 60 мин
      - Отправка данных об ошибке в сервис Sentry, 120 мин
      
  - Использование websocket провайдера для внутреннего кошелька
